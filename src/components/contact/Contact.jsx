...
export default function ContactPage() {
	...
	const [showrechpt, setShowRecpha] = useState(false);
	const [errors, setErrors] = useState({});
	const [status, setStatus] = useState(null);
	...
	const handleSubmit = async (e) => {
		...
		try {
			...
			const response = await fetch(
				`https://admin.monsinidress.com/api/contact`,
				{
					method: "POST",
					headers: {
	...
	const getInputClasses = (field) => {
		const base =
			"mt-1 block w-full rounded-lg border px-4 py-3 shadow-sm transition-shadow focus:outline-none focus:ring-2 focus:ring-offset-0 focus:ring-zinc-800 bg-white placeholder-gray-400";
		return errors[field]
			? `${base} border-red-500`
			: `${base} border-gray-300 hover:border-gray-400`;
	};
					...
					<section className="bg-white p-10 rounded-xl shadow-2xl border border-gray-200">
						...
						<form onSubmit={handleSubmit} className="space-y-6">
							...
							<div className="pt-2 flex items-center space-x-4">
								<button
									type="submit"
									className="inline-flex items-center text-white justify-center rounded-lg bg-zinc-800 px-6 py-3 text-base font-bold shadow-md hover:bg-zinc-700 transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-zinc-800 disabled:opacity-50 disabled:cursor-not-allowed"
									disabled={status === "sending"}>
									{status === "sending" ? (
										<>
											<svg
